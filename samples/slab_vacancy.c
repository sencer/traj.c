...
void GetCosts(Crystal *c, CoarseBox *box, int atom, int boxid, int target_nat, int costs[target_nat][c->nat-target_nat])
{
  ...
  for (int i = 0; i < box->binsn[boxid]; ++i)
  {
    ...
    if ( atom2 >= target_nat)
    {
      ...
      {
        costs[atom][atom2-target_nat] = CrystalDist(c, c->atoms[atom].coor, c->atoms[atom2].coor);
      }
      else
      {
...
int main(int argc, char *argv[])
{
  ...
  int target_nat, ref_nat, nmapped, frame=0, t;
  double dm[3];
  ...
  while(!feof(target))
  {
    ...
    // Print varxyz file.
    fprintf(out1, "%d\ncelldm %10.6f %10.6f %10.6f 90 90 90\n", target_nat - nmapped, dm[0], dm[1], dm[2]);
    for (int i = 0; i < target_nat; ++i)
    {
      if(atomMap[i] == -1)
    ...
    // Print varxyz file.
    fprintf(out2, "%d\ncelldm %10.6f %10.6f %10.6f 90 90 90\n", ref_nat - nmapped, dm[0], dm[1], dm[2]);
    ...
    c->nat = target_nat;                             //debug

    /* for (int i = 0; i < 19; ++i)                     //debug */
    /* {                                                //debug */
    /*   LMPReadHeader(target, &t, &target_nat, dm);    //debug */
    /*   LMPReadFrame(target, c);                       //debug */
    /*   frame++;                                       //debug */
    /* }                                                //debug */
    LMPReadHeader(target, &t, &target_nat, dm);
    frame++;                                       //debug
