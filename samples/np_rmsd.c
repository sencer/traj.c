...
void Transform(Crystal *c)
{
  ...
  for (int i = 0; i < c->nat; ++i)
  {
    ...
    tmp[1] =                  0.667*coor[1] - 0.745*coor[2] + 80.877;
...
int main(int argc, char *argv[])
{
  ...
  do
  {
    ...
    fprintf(fout, "%d\ncelldm %f %f %f\n", tar->nat, 2*CENTER, 2*CENTER, 2*CENTER);
    ...
    for (int ir = 0; ir < tar->nat; ++ir)
    {
      ...
      isurf = argmin(sdist, nsurf);

      if ( sdist[isurf] > 3 )
      {
        // out of the NP
        fprintf(fout, "%-2s %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %2d\n",
            PT_Symbol(at.Z), at.coor[0], at.coor[1], at.coor[2], beta(at.coor),
            sdist[isurf], at.id/100.0, -1);
      }
      else if (sdist[isurf] < -8)
      {
        // bulk, not label as close to a particular surface
        fprintf(fout, "%-2s %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %2d\n",
            PT_Symbol(at.Z), at.coor[0], at.coor[1], at.coor[2], beta(at.coor),
            sdist[isurf], at.id/100.0, 0);
      }
      else
      {
        // close to a surface
        fprintf(fout, "%-2s %10.6f %10.6f %10.6f %10.6f %10.6f %10.6f %2d\n",
            PT_Symbol(at.Z), at.coor[0], at.coor[1], at.coor[2], beta(at.coor),
            sdist[isurf], at.id/100.0, equiv[isurf]);
      }
    }
  }
  while (eof!=EOF);
